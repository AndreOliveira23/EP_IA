...
file_path = 'p_stats_2020.csv'  # Update with the actual path
...
feature_indices = [header.index(col) for col in ['latitude', 'longitude', 'ano', 'nup', 'ambiente']]
target_index = header.index('medp')
...
# Classify target variable based on the given ranges for phosphorus concentration
def classify_phosphorus(value):
    if value <= 0.10:
        return 1  # Classe 1 ou 2 (Low Phosphorus)
    elif value <= 0.15:
        return 2  # Classe 3 ou 4 (Moderate Phosphorus)
    else:
        return 3  # High Phosphorus

y_classified = [classify_phosphorus(value) for value in y]
...
best_k = 5  # You can choose the best k value based on the metrics
knn_classifier = KNeighborsClassifier(n_neighbors=best_k)
knn_classifier.fit(X_train_scaled, y_train)
y_pred = knn_classifier.predict(X_test_scaled)
conf_matrix = confusion_matrix(y_test, y_pred)
disp = ConfusionMatrixDisplay(confusion_matrix=conf_matrix, display_labels=['Classe 1 ou 2', 'Classe 3 ou 4', 'Sem classificação'])
disp.plot(cmap=plt.cm.Blues)
plt.title('Matriz de Confusão para \'Fósforo Total\'')
plt.show()
