...
file_path = 'dbo_stats_2020.csv'  # Update with the actual path
...
feature_indices = [header.index(col) for col in ['latitude', 'longitude', 'ano', 'nudbo', 'ambiente']]
target_index = header.index('meddbo')
...
y_classified = [classify_meddbo(value) for value in tqdm(y, desc="Classifying target")]
